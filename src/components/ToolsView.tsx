    ...
    <div className="view-stack">
      <section className="page-intro tools-intro">
        <div>
          ...
          <h1>ثلاث أدوات.<br />ولا واحدة منها إجبارية.</h1>
          <p>جرّب الأداة التي تجعل انتباهك أوسع. لو أداة زادت التوتر، أوقفها واستخدم أخرى أو ارجع لنشاط بسيط وآمن.</p>
        </div>
        <div className="tool-orbs" aria-hidden="true">
          <span><Icon name="wind" /></span>
      ...
      <div className="tool-tabs" role="tablist" aria-label="أدوات التنظيم">
        <button className={activeTool === 'breathing' ? 'is-active' : ''} type="button" onClick={() => setActiveTool('breathing')} role="tab" aria-selected={activeTool === 'breathing'}>
          <Icon name="wind" size={18} />
          التنفّس
          <span>هدوء الإيقاع</span>
        </button>
        <button className={activeTool === 'grounding' ? 'is-active' : ''} type="button" onClick={() => setActiveTool('grounding')} role="tab" aria-selected={activeTool === 'grounding'}>
          <Icon name="compass" size={18} />
          الحواس
          <span>عودة للحاضر</span>
        </button>
        <button className={activeTool === 'relaxation' ? 'is-active' : ''} type="button" onClick={() => setActiveTool('relaxation')} role="tab" aria-selected={activeTool === 'relaxation'}>
      ...
      <section className="choose-tool-guide">
        ...
        <div className="guide-grid">
          <article>
            <span className="guide-icon mint"><Icon name="wind" /></span>
            <h3>النفس سريع أو سطحي</h3>
            <p>ابدأ بالتنفّس الخفيف. لا تأخذ شهيقًا عميقًا بالقوة.</p>
          </article>
          <article>
            <span className="guide-icon lilac"><Icon name="compass" /></span>
            <h3>عقلك يفتش عن تهديد</h3>
            <p>استخدم الحواس وحدد ما هو موجود فعلًا في المكان الآن.</p>
          </article>
          <article>
            <span className="guide-icon peach"><Icon name="sparkles" /></span>
